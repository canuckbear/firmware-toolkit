# u-boot installation for generic-amd64
...
. The commands are run on the target board (generic-amd64)
...
# Copy binary to MMC and thus upadate u-boot (environnment variables stored on MMC are not modified)
sudo dd if=/boot/u-boot/u-boot-generic-amd64 of=/dev/mmcblk0 bs=1024 seek=8 status=noxfer
