# u-boot installation for orangepi-one-plus
...
/boot/u-boot/u-boot-orangepi-one-plus

This is a symlink to the current u-boot file stored at the same location in a
file named u-boot-orangepi-one-plus.VERSION
...
. The commands are run on the target board (orangepi-one-plus)
...
sudo dd if=/boot/u-boot/u-boot-orangepi-one-plus of=/dev/mmcblk0 bs=1024 seek=8 status=noxfer
