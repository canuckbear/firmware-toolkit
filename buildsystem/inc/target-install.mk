			...
			if [ ! -f $(COOKIE_DIR)/$@ ] ; then \
				if [ "$(SW_NAME)" = "u-boot" ] ; then \
					...
					if [ "$(SW_NAME)" = "linux" ] ; then \
						...
						$(BUILD_ENV) $(MAKE) INSTALL_PATH=$(INSTALL_DIR)/boot $(LINUX_KERNEL_COMMON_INSTALL_ARGS) ; \
						$(BUILD_ENV) $(MAKE) INSTALL_MOD_PATH=$(INSTALL_DIR)/ INSTALL_MOD_STRIP=1 modules_install ; \
						echo "DEBUG : le DEFAULT_DTB $(DEFAULT_DTB)" ; \
			 	 		if [ ! "" = "$(DEFAULT_DTB)" ] ; then \
							find $(BUILD_DIR)/arch/ -name $(DEFAULT_DTB) -exec  cp -v {} $(INSTALL_DIR)/boot/dtb/ \; ;\
							ln -sf dtb/$(DEFAULT_DTB) $(INSTALL_DIR)/boot/default.dtb ; \
						else \
							find $(BUILD_DIR)/arch/ -name "*.dtb" -exec cp -v {} $(INSTALL_DIR)/boot/dtb/ \; ; \
						fi ; \
					fi ; \
				fi ; \
			fi ; \
